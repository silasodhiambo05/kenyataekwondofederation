...
if ($_POST['askQuestion']) {
	?>
        <script>
        	document.getElementById("manage_panel").innerHTML="";
        </script>
        <div class="row">
        <div class="col-lg-2">
        </div>
...
if ($_POST['btnSubmitMessage']) {
	...
	$sql1 = "INSERT INTO `athlete_messages` 
...
if ($_POST['myInbox']) {
	?>
        ...
        <?php
	$sql="SELECT subject,message,date_sent FROM athlete_messages WHERE ktf_number='$dan_number'";
        ...
        $sql="SELECT subject,message,date_sent,reply,date_reply FROM athlete_messages WHERE ktf_number='$dan_number'";
                  $result=mysqli_query($con,$sql) or die(mysql_error());
                  ?>
