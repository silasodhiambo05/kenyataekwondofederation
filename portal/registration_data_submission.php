...
if ($_POST['submit_personal']) {
  ...
  $gender=$_POST['gender'];
    $index="";
    $dbyear="";
    $sqls="SELECT * FROM  athlete WHERE id_number='$id_number'";
    ...
    $sqlsc = "SELECT number,year FROM dan_log";
        ...
        if(strlen($num)===3)
        {
          $zero='0';
				...
				$dan='KTF/ATH/'.$zero.$num;
				$pass=$num.$dbyear;
if (empty($nid)===false) {
  ?>
  <script>
alert("Sorry! Person with this national id is already registered.");
</script>
  <?php
}
else
{
				$sql1 = "INSERT INTO `athlete` 
...
{
  echo '<h5 class="btn-warning">We are Unable to send Message in your email. Check your network connection</h5>';
  //echo 'Mail error' . $mail->ErrorInfo;
  
  
}
											?>
		                                 <h3 class="btn-success"><span class="glyphicon glyphicon-info-sign"></span> Registration application success. Pay registration fees of Kshs.1000 to business number 290020, account kenyataekwondo to get a KTF number. Ensure you pay using Mpesa number you used to register.</h3>
		                                    
		                                  <?php 
		                                  $sqz=mysqli_query($con,"UPDATE dan_log SET number='$num'");
