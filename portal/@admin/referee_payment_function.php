...
if ($_POST['submit_payment']) {
	...
	date_default_timezone_set('Africa/Nairobi');
      ...
      while($row=mysqli_fetch_array($result))
       ...
       $giver='Referee ID Number '.$national_id;
				...
				(national_id,purpose,amount,ref_number,date_paid,status) VALUES ('$national_id','$reason','$amountPaid','$ref_number','$date','1')";
	                            ...
	                            if (!mysqli_query($con,$sql1)) {
		                          ...
		                          echo 'Not received';
										...
										{
											$sqz=mysqli_query($con,"UPDATE referee SET status='1' WHERE id_number='$national_id'");
		                                 ...
		                                 <h3 class="btn-success"><span class="glyphicon glyphicon-info-sign"></span> Payment of  <?php echo 'Kshs.'.$amountPaid.' reference number '.$ref_number;?> for <?php echo $name;?> Received successifully.</h3>
		                                  <?php
		                                  $sqz=mysqli_query($con,"UPDATE `referee` SET `status`='1' WHERE national_id='$national_id'");
		            //--------------------------------------------------------------------------messsage
...
$mail->Body = 'You have successifully registered as referee in kenya taekwondo federation<br>Your KTF Number:'.$ktf.'<br> Your Password:'.$password.'<br>Loging to kenyataekwondo.co.ke/portal/referee_login.php to view and update your profile<br> Thanks for registering with Kenya Taekwondo Federation. Visit our website kenyataekwondo.co.ke<br>KENYA TAEKWONDO FEDERATION (KTF)<br>
