<?php
if ($_POST['approve_event_applications']) {
	?>
        <script>
        	document.getElementById("magement_panel").innerHTML="";
        </script>
        <?php
        $sql="SELECT id,event_id,ktf_number,applicant_name,date_applied FROM events_application WHERE status='Not Approved'";
                  $result=mysqli_query($con,$sql) or die(mysql_error());
                  ?>
                  <h3>List of applications for various events</h3>
    ...
    <span class="input-group-addon glyphicon glyphicon-search"></span>
        <input type="text" onkeyup="myFunction()" id="myInput" class="form-control" placeholder="Enter ktf number to search"  style="width:60%; height:30px; float: center; ">
                  ...
                  <div id="table-scroll">
                 <table  id="table_appications" class="table table-bordered table-striped table-hover table-responsive">
             <thead>    
       ...
       <th>Approve</th>
         ...
         <td style="background-color:lightgreen;" ><a href="approve_reject_application.php?c=<?php echo $id;?>"><button type='submit' class='btn btn-primary' name='viewPhone'><span class='glyphicon glyphicon-check'></span> Approve/Reject</button></a></td>
         </tr>
         </tbody>
...
<script type="text/javascript">
	function myFunction() {
        ...
        table = document.getElementById("table_appications");
